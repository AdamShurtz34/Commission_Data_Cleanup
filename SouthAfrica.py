...
def ExecuteSouthAfrica(fileName):
  wb = Workbook()
  ws = wb.active
  ws.title = "South Africa Commission Payout"
  sumCom = 0
  ...
  rowIndex=2
  bankNamesandSwift = {"First National Bank":["FIRNZAJJ", "FIRNZAJJXXX"], "Capitec Bank Limited":["CABLZAJJ"],
                       "FNB":["FIRNZAJJ", "FIRNZAJJXXX"], "NEDBANK":["NEDSZAJJ"], "Nedbank":["NEDSZAJJ"]}
  today = date.today()
  rd = REL.relativedelta(days=1, weekday=REL.FR)
  next_friday = today + rd
  today = next_friday.strftime("%#m-%#d-%Y")
  file = open("SouthAfrica_change_log_" + today + ".txt", "a")
      ...
      if rowIndex == 1:
        ...
        if colIndex==11:
          if cell.value != "ZA":
            oldBC = cell.value
            cell.value = "ZA"
            file.write("ID: " + str(username.value) + ", bank Country changed to ZA" + " from ''" + oldBC + "''"  +"\n")
  ...
  wb.save("South Africa Commission Payout File " + today + ".xlsx")
  file.close
