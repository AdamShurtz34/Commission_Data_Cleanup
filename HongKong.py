...
def ExecuteHongKong(fileName):
  wb = Workbook()
  ws = wb.active
  ws.title = "Hong Kong Commission Payout"
  sumCom = 0
  ...
  rowIndex=2
  bankNamesandSwift = {"Bank of China":["BKCHHKHH"], "Boc":["BKCHHKHH"], "BOC":["BKCHHKHH"], "DBS Bank":["DHBKHKHH"],
                       "China Citic Bank":["KWHKHKHHXXX"], "Dah Sing Bank":["DSBAHKHH"], "Hang Seng Bank":["HASEHKHH"],
                       "HSBC":["HSBCHKHHHKH"], "Hsbc":["HSBCHKHHHKH"], "Industrial & Commercial Bank Of China":["ICBKHKHH"],
                       "Shanghai Commercial Bank":["SCBKHKHH"], "Standard Chartered Bank":["SCBLHKHH"],
                       "Nanyang Commercial Bank":["NYCBHKHH"], "Wing Hang Bank":["WIHBHKHH"],
                       "Oversea Chinese Banking Corp":["OCBCHKHHXXX"], "Chong Hing Bank": ["LCHBHKHH"],
                       "Wing Lung Bank":["WUBAHKHH"], "CHIYU Banking Corporation Limited":["CIYUHKHHXXX","CIYUHKHH"],
                       "Bank of East Asia":["BEASHKHH"], "China Citic Bank":["KWHKHKHH"], "China Citic Bank International":["KWHKHKHH"]}
  today = date.today()
  rd = REL.relativedelta(days=1, weekday=REL.FR)
  next_friday = today + rd
  today = next_friday.strftime("%#m-%#d-%Y")
  file = open("HongKong_change_log_" + today + ".txt", "a")
      ...
      if rowIndex == 1:
        ...
        if colIndex==11:
          if cell.value != "HK":
            oldBC = cell.value
            cell.value = "HK"
            file.write("ID: " + str(username.value) + ", bank Country changed to HK" + " from ''" + oldBC + "''"  +"\n")
        ...
        if colIndex==13:
          if cell.value != "hkd":
            oldC = cell.value
            cell.value = "hkd"
            file.write("ID: " + str(username.value) + ", currency changed to hkd" + " from ''" + oldC + "''"  +"\n")
  ...
  wb.save("Hong Kong Commission Payout File " + today + ".xlsx")
  file.close
