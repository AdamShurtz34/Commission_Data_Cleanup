...
def ExecutePhilippines(fileName):
  wb = Workbook()
  ws = wb.active
  ws.title = "Philippines Commission Payout"
  sumCom = 0
  ...
  rowIndex=2
  bankNamesandSwift = {"Asia United":["AUBKPHMM"], "BANK OF THE PHILIPPINE ISLANDS":["BOPIPHMM"], "China Banking Corp":["CHBKPHMM"], 
                       "Bank of the Philippine Islands":["BOPIPHMM"], "Bank Of The Philippine Islands":["BOPIPHMM"],
                       "Bank of the Philippine Islands":["BOPIPHMM"], "Bdo Unibank":["BNORPHMM"],
                       "BDO Unibank Inc":["BNORPHMM"], "CTBC Bank":["CTCBPHMM"], "ONE Network Bank":["ONNRPHM1"],
                       "Development Bank of the Philippines":["DBPHPHMM"], "East West Bank":["EWBCPHMM"],
                       "Eastwest bank":["EWBCPHMM"], "GCASH":["NA"], "Land Bank of the Philippines":["TLBPPHMM"],
                       "Metropolitan Bank and Trust":["MBTCPHMM"], "Metropolitan Bank and Trust Co":["MBTCPHMM"],
                       "Philippine Savings Bank":["PHSBPHMM"], "PayMaya":["PAPHPHM1"], "PayMaya Phillipines Inc":["PAPHPHM1"],    
                       "Rizal Commercial":["RCBCPHMM"], "Philippine National":["PNBMPHMM"], "Security Bank Corporation":["SETCPHMM"]}
  today = date.today()
  rd = REL.relativedelta(days=1, weekday=REL.FR)
  next_friday = today + rd
  today = next_friday.strftime("%#m-%#d-%Y")
  file = open("Philippines_change_log_" + today + ".txt", "a")
      ...
      if rowIndex == 1:
        ...
        if colIndex==11:
          if cell.value != "PH":
            oldBC = cell.value
            cell.value = "PH"
            file.write("ID: " + str(username.value) + ", bank Country changed to PH" + " from ''" + oldBC + "''"  +"\n")
        ...
        if colIndex==13:
          if cell.value != "php":
            oldC = cell.value
            cell.value = "php"
            file.write("ID: " + str(username.value) + ", currency changed to php" + " from ''" + oldC + "''"  +"\n")
  ...
  wb.save("Philippines Commission Payout File " + today + ".xlsx")
  file.close
