...
def ExecuteTaiwan(fileName):
  wb = Workbook()
  ws = wb.active
  ws.title = "Taiwan Commission Payout"
  sumCom = 0
  ...
  file = open("Taiwan_change_log_" + today + ".txt", "a")
      ...
      if rowIndex == 1:
        ...
        if colIndex==11:
          if cell.value != "TW":
            oldBC = cell.value
            cell.value = "TW"
            file.write("ID: " + str(username.value) + ", bank Country changed to TW" + " from ''" + oldBC + "''"  +"\n")
        ...
        if colIndex==13:
          if cell.value != "twd":
            oldC = cell.value
            cell.value = "twd"
            file.write("ID: " + str(username.value) + ", currency changed to twd" + " from ''" + oldC + "''"  +"\n")
  ...
  wb.save("Taiwan Commission Payout File " + today + ".xlsx")
  file.close
