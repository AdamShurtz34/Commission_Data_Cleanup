...
def ExecuteHungary(fileName):
  wb = Workbook()
  ws = wb.active
  ws.title = "Hungary Commission Payout"
  sumCom = 0
  ...
  rowIndex=2
  bankNamesandSwift = {"Budapest Bank":["BUDAHUHB"], "Budapest Bank Zrt":["BUDAHUHB"], "CIB Bank":["CIBHHUHB", "CIBHHUHBXXX"],
                       "Cib Bank":["CIBHHUHB", "CIBHHUHBXXX"], "Erste Bank":["GIBAHUHB"], "K&H":["OKHBHUHB"], "K&H Bank":["OKHBHUHB"],
                       "K&H Bank Zrt":["OKHBHUHB"], "Magyar Takarekszovetkezeti Bank":["TAKBHUHB"], "OTP Bank": ["OTPVHUHB"],
                       "OTP Bank Zrt": ["OTPVHUHB"],"OTP Bank Nyrt":["OTPVHUHB"], "Raiffeisen Bank":["UBRTHUHB"] ,"Sopron Bank":["EHBBHUHS"],
                       "Takarekbank":["FHKBHUHB"], "UniCredit Bank Hungary":["BACXHUHB"], "UniCredit Bank":["BACXHUHB"],
                       "Magnet Bank":["HBWEHUHB"], "CIB Bank Zrt":["CIBHHUHB", "CIBHHUHBXXX"], "UniCredit Bank Hungary Zrt":["BACXHUHB"],
                       "Kereskedelmi es Hitelbank Zrt.":['OKHBHUHB']}
  today = date.today()
  rd = REL.relativedelta(days=1, weekday=REL.FR)
  next_friday = today + rd
  today = next_friday.strftime("%#m-%#d-%Y")
  file = open("Hungary_change_log_" + today + ".txt", "a")
    ...
    for cell in row:
      if rowIndex == 1:
        ...
        if colIndex==8:
          ...
          cell.value = acc
          if value != acc:
            file.write("ID: " + str(username.value) + ", account number was changed to " + acc + " from ''" + value + "''"  +"\n")
          if len(cell.value) != 28:
            cell.fill = PatternFill(fill_type='solid', start_color='f8ff94', end_color='f8ff94')
        ...
        if colIndex==11:
          if cell.value != "HU":
            oldBC = cell.value
            cell.value = "HU"
            file.write("ID: " + str(username.value) + ", bank Country changed to HU" + " from ''" + oldBC + "''"  +"\n")
        ...
        if colIndex==13:
          if cell.value != "huf":
            oldC = cell.value
            cell.value = "huf"
            file.write("ID: " + str(username.value) + ", currency changed to huf" + " from ''" + oldC + "''"  +"\n")
  ...
  wb.save("Hungary Commission Payout File " + today + ".xlsx")
  file.close
