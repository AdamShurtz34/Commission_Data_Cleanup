...
def ExecuteUkraine(fileName):
  wb = Workbook()
  ws = wb.active
  ws.title = "Ukraine Commission Payout"
  sumCom = 0
  ...
  rowIndex=2
  bankNamesandSwift = {"PrivatBank":["14360570"], "Oschadbank":["00032129"], "Express Bank":["20053145"],
                       "Приват Банк":["14360570"], "Alfa Bank":["23494714"], "АТ Универсал банк":["21133352"],
                       "Универсал банк":["21133352"], "АТ Альфа-банк":["23494714"], "Raiffeisen Bank":["14305909"],
                       "ПУМБ":["14282829"]}
                       #EDRPOU - the number given with each bank name, is like a routing number in Ukraine
  
  today = date.today()
  rd = REL.relativedelta(days=1, weekday=REL.FR)
  next_friday = today + rd
  today = next_friday.strftime("%#m-%#d-%Y")
  file = open("Ukraine_change_log_" + today + ".txt", "a")
    ...
    for cell in row:
      if rowIndex == 1:
        ...
        if colIndex==8:
          if len(cell.value) == 0:
            account.fill = PatternFill(fill_type='solid', start_color='f8ff94',   end_color='f8ff94')
          elif len(cell.value) == 29:
            oldV = account.value
            Iban = account.value[10:]
            account.value = Iban
            file.write("ID: " + str(username.value) + ", account # changed to " + account.value + " from ''" + oldV + "''"  +"\n")
          elif len(account.value) != 19:
            isNum = True
            try:
              int(routing.value)
            except ValueError:
            ...
            if not isNum:
              ...
              zeros = "0"
              accLength = len(account.value)
              numOfZeros = 19 - accLength
              zeros = zeros*numOfZeros
              oldV = account.value
              accountNum = zeros + oldV
              account.value = accountNum
              file.write("ID: " + str(username.value) + ", account # changed to " + account.value + " from ''" + oldV + "''"  +"\n")
          else:
            isNum = True
            try:
        ...
        if colIndex==11:
          if cell.value != "UA":
            oldBC = cell.value
            cell.value = "UA"
            file.write("ID: " + str(username.value) + ", bank Country changed to UA" + " from ''" + oldBC + "''"  +"\n")
        ...
        if colIndex==13:
          if cell.value != "uah":
            oldC = cell.value
            cell.value = "uah"
            file.write("ID: " + str(username.value) + ", currency changed to uah" + " from ''" + oldC + "''"  +"\n")
  ...
  wb.save("Ukraine Commission Payout File " + today + ".xlsx")
  file.close
