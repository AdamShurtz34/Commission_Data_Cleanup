...
def ExecuteBotswana(fileName):
  wb = Workbook()
  ws = wb.active
  ws.title = "Botswana Commission Payout"
  sumCom = 0
  ...
  rowIndex=2
  bankNamesandSwift = {"ABSA Bank":["BARCBWGX"], "ABSA BANK":["BARCBWGX"], "Absa Bank":["BARCBWGX"], "Absa Bank of Botswana":["BARCBWGX"],
                       "Banc Abc":["FMBZBWGA"], "Barclays Bank":["BARCBWGX"], "Barclays Bank Botswana":["BARCBWGX"],
                       "Barclays Bank of Botswana":["BARCBWGX"], "First National Bank":["FIRNBWGX", "FIRNBWGXXXX"],
                       "First National Bank Botswana":["FIRNBWGX", "FIRNBWGXXXX"],
                       "First National Bank Botswana Limited":["FIRNBWGX", "FIRNBWGXXXX"], "FNB":["FIRNBWGX"],
                       "Stanbic":["SBICBWGX", "SBICBWGXXXX"], "STANBIC":["SBICBWGX", "SBICBWGXXXX"],
                       "Stanbic Bank":["SBICBWGX", "SBICBWGXXXX"], "Stanbic Bank Botswana":["SBICBWGX", "SBICBWGXXXX"],
                       "Standard Chartered":["SCHBBWGX", "SCHBBWGXXX"],
                       "Standard Chartered Bank":["SCHBBWGX", "SCHBBWGXXX"],
                       "Standard Chartered Bank Botswana":["SCHBBWGX", "SCHBBWGXXX"],
                       "STANDARD CHARTERED BANK":["SCHBBWGX", "SCHBBWGXXX"]}
  today = date.today()
  rd = REL.relativedelta(days=1, weekday=REL.FR)
  next_friday = today + rd
  today = next_friday.strftime("%#m-%#d-%Y")
  file = open("Botswana_change_log_" + today + ".txt", "a")
    ...
    for cell in row:
      if rowIndex == 1:
        ...
        if colIndex==8:
          if cell.value == "":
            cell.fill = PatternFill(fill_type='solid', start_color='f8ff94', end_color='f8ff94')
          if len(cell.value) != 13 and len(cell.value) != 11 and len(cell.value) != 7:
            cell.fill = PatternFill(fill_type='solid', start_color='f8ff94', end_color='f8ff94')
          isNum = True
          try:
            int(cell.value)
        ...
        if colIndex==11:
          if cell.value != "BW":
            oldBC = cell.value
            cell.value = "BW"
            file.write("ID: " + str(username.value) + ", bank Country changed to BW" + " from ''" + oldBC + "''"  +"\n")
  ...
  wb.save("Botswana Commission Payout File " + today + ".xlsx")
  file.close
