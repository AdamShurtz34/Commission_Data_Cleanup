...
def ExecuteSingapore(fileName):
  wb = Workbook()
  ws = wb.active
  ws.title = "Singapore Commission Payout"
  sumCom = 0
  ...
  rowIndex=2
  bankNamesandSwift = {"DBS":["DBSSSGSG"], "DBS Bank":["DBSSSGSG"], "DBS Bank LTD":["DBSSSGSG"], "Maybank":["MBBESGS2"],
                       "OCBC":["OCBCSGSG"], "POSB":["DBSSSGSG"], "UOB":["UOVBSGSG"], "United Overseas Bank":["UOVBSGSG"],
                       "Standard Chartered Bank":["SCBLSG22"], "CIMB":["CIBBSGSG"]}
  today = date.today()
  rd = REL.relativedelta(days=1, weekday=REL.FR)
  next_friday = today + rd
  today = next_friday.strftime("%#m-%#d-%Y")
  file = open("Singapore_change_log_" + today + ".txt", "a")
    ...
    for cell in row:
      if rowIndex == 1:
        ...
        if colIndex==6:
          ...
          bankName.value = name
          if bName != name:
            file.write("ID: " + str(username.value) + ", bank name changed to " + name + " from ''" + bName + "''"  +"\n")
        ...
        if colIndex==11:
          if cell.value != "SG":
            oldBC = cell.value
            cell.value = "SG"
            file.write("ID: " + str(username.value) + ", bank Country changed to SG" + " from ''" + oldBC + "''"  +"\n")
        ...
        if colIndex==13:
          if cell.value != "sgd":
            oldC = cell.value
            cell.value = "sgd"
            file.write("ID: " + str(username.value) + ", currency changed to sgd" + " from ''" + oldC + "''"  +"\n")
  ...
  wb.save("Singapore Commission Payout File " + today + ".xlsx")
  file.close
